...
public class WeaponSwipe : WeaponBase {
    private float MaxPathDistance => Mathf.Max(_dataWeaponSwipe.maxPathDistance, ManagerInput.MinSwipeDistance);
    ...
    private void TrySwipeAttack() {
        // Only proceed if at least 2 swipe points are in array or if weapon doesn't have enough energy.
        if (PointCount < 2 || !TryUseEnergy(EnergyCost)) {
            ResetPath(true);
            return;
        }
    ...
    bool WillPointBeValid(Vector3 point) {
        ...
        return Vector3.Distance(point, LastPoint) >= ManagerInput.MinSwipeDistance;
    }
