...
public class ZombieBase : Damageable {
    ...
    readonly Collider[] _hitCol = new Collider[1];
    void CheckForSideTarget() {
        Vector3 capsuleTop = Position + (Vector3.up * _nav.height);

        // Check if a valid transform overlaps with zombie. If a new side target was set from found transform, stop code here.
        if (Physics.OverlapCapsuleNonAlloc(Position, capsuleTop, CharacterRadius, _hitCol, _sideTargetMask) > 0)
            if (TrySetSideTarget(_hitCol[0].transform))
                return;

        bool approaching = _zombieState is ZombieState.Approaching;
        // If zombie is approaching, check towards their move direction. Otherwise, check towards their main target.
        Vector3 targetDir = approaching ? _nav.desiredVelocity.normalized : (_mainTarget.Position - Position).normalized;
        float checkDist = approaching ? 0.1f : Mathf.Min(GetDistanceToMainTarget(), _attackRange);

        // Check if a valid transform is in front of zombie within given range.
        if (Physics.CapsuleCast(Position, capsuleTop, CharacterRadius, targetDir, out RaycastHit hit, checkDist, _sideTargetMask))
            TrySetSideTarget(hit.transform);
    }

    /// Attempts to set side target to given target. Returns true and updates zombie state and  if it was successful.
    private bool TrySetSideTarget(Transform possibleTarget) {
        // False if transform doesn't have damageable component.
        if (!possibleTarget.TryGetComponent(out Damageable damageable)) return false;
        // False if damageable is either main target or current side target.
        if (damageable == _mainTarget || damageable == _sideTarget) return false;
        // Update side target, update zombie state, and return true.
        _sideTarget = damageable;
