...
public class ZombiesPool {
    ...
    public ZombiesPool(ManagerZombies manager, GameObject zombieBasePrefab, Health mainTarget, float approachDist, LayerMask sideTargetMask) {
        Manager = manager;
    ...
    public ZombieCore Pop(DataZombie dataZombie, Vector3 spawnPos) {
        if (_zombieStack.Count <= 0) ExpandPool();
        ZombieCore z = _zombieStack.Pop();
        z.Pop(dataZombie, spawnPos);
        return z;
    }
    ...
    public void DepletePool(ManagerZombies manager) {
