...
public class ZombiesManager : MonoBehaviour {
    ...
    private event Action<int, float> FixedUpdateCustom;
    ...
    public ZombieData[] zombieDataEntries;
    ...
    void Start() {
        ...
        _mainTarget = PoorSoul.Instance;
        _zombiesPool = new ZombiesPool(this, zombieBasePrefab, _mainTarget, _approachDistance, _sideTargetMask);
        
        _zombiesWaveManager.OnHordeSpawn += SpawnZombieHorde;
        _zombiesWaveManager.OnTrickleSpawn += SpawnZombieRandom;
        _zombiesWaveManager.OnAllZombiesDead += DepletePool;
    }
    ...
    void FixedUpdate() {
        FixedUpdateCustom?.Invoke(_zombieTick, Time.fixedDeltaTime);
        _zombieTick++;
    }
    ...
    public ZombieCore SpawnZombieRandomWithReturn(int currentWave) {
        ZombieData data = GetZombieData(currentWave, out int index);
        if (!data) return null;
    ...
    private ZombieCore SpawnZombieSpecific(int dataIndex) {
        ZombieData data = zombieDataEntries[Mathf.Clamp(dataIndex, 0, zombieDataEntries.Length - 1)];
    ...
    private ZombieData GetZombieData(int currentWave, out int selectedIndex) {
        selectedIndex = -1;
        float totalWeight = 0;
        ...
        for (int i = 0; i < zombieDataEntries.Length; i++) {
            ZombieData types = zombieDataEntries[i];
        ...
        foreach (int i in validEntries) {
            ZombieData zombie = zombieDataEntries[i];
